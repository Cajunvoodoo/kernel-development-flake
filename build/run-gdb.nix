...
let
  moduleDirs = builtins.map (m: "${m}/lib/modules/${kernel.modDirVersion}/misc") modules;

  moduleSourceDirs = builtins.map (m: m.src) modules;

  symbolsDirs = builtins.concatStringsSep " " moduleDirs;
